...
export class WebserviceFetcher<Params, T> {
  ...
  public constructor(
    ...
    if (typeof params.rowVon != 'number' || typeof params.rowBis != 'number' || !parser.parametersGuard(params)) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        `Invalid anpfiff.info parameters: ${JSON.stringify(params)}`,
      );
    }
    this.expectedLength = params.rowBis - params.rowVon + 1;
  ...
  private async setCache(value: { hasMoreData: boolean; value: T[] }): Promise<void> {
    const cacheRef = admin.database().ref(`caches/${normalizeRefPathComponent(this.url)}`);
    await cacheRef.set({
